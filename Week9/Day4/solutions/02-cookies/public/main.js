...
function restoreTheme() {
  ...
  const themeName = cookieObject["themeName"];
...
function clearTheme() {
  ...
  document.cookie = `themeName=; expires=${now}`;
...
const cookieParser = () => {
  ...
  const mappedCookie = splitCookie.map((cookie) => {
    return cookie.split("=");
  });

  console.log("mapped cookie ", mappedCookie);
  mappedCookie.forEach((cookie) => {
...
function restoreName() {
  ...
  const displayName = cookieObject["displayName"];
...
function clearName() {
  ...
  document.cookie = `displayName=; expires=${now}`;
}
